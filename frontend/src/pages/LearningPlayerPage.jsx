                ...
                <div className="space-y-3">
                  <div className="w-full aspect-video bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl relative">
                    {(() => {
                      ...
                      if (!url) {
                        return <div className="flex items-center justify-center h-full text-slate-500 text-xs font-bold">No video URL provided</div>;
                      }
                      
                      const isDirectVideo = url.toLowerCase().endsWith('.mp4') || url.toLowerCase().endsWith('.webm');
                      
                      if (isDirectVideo || url.includes('cloudinary.com')) {
                        return (
                          <video
                            src={url}
                            controls
                  ...
                  {activeLesson?.videoUrl && !activeLesson.videoUrl.includes('cloudinary.com') && !activeLesson.videoUrl.toLowerCase().endsWith('.mp4') && (
                    <div className="flex items-center justify-between px-4 py-2.5 bg-slate-900/50 rounded-xl border border-slate-800 text-xs text-slate-400">
                      <span>Having trouble playing? This video cannot be embedded here.</span>
                      <a 
                      ...
                      >
                        Open on Source Website <ExternalLink className="w-3.5 h-3.5" />
                      </a>
                    </div>
                  )}
                </div>
